...
export class PdfService {
  ...
  generateResumePDF(resumeId: string, templateId?: string): Observable<Blob> {
    const url = `${this.apiUrl}/pdf/generate/${resumeId}`;
  ...
  generatePDFFromData(
    ...
    const url = `${this.apiUrl}/pdf/generate-direct`;
  ...
  generateResumePreview(
    ...
    const url = `${this.apiUrl}/pdf/preview/${resumeId}`;
    let params = new HttpParams();
    if (templateId) {
      params = params.set('template', templateId);
    }
  ...
  checkPDFServiceHealth(): Observable<any> {
    const url = `${this.apiUrl}/pdf/health`;
